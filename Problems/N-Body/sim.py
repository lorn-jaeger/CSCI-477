...
def RungeKutta45(dt, f, t, y, *args):
    ...
    k6 = f(
        t + c6 * dt,
        y + dt * (a61 * k1 + a62 * k2 + a63 * k3 + a64 * k4 + a65 * k5),
        *args,
    )
    k7 = f(
        t + dt,
        y + dt * (a71 * k1 + a72 * k2 + a73 * k3 + a74 * k4 + a75 * k5 + a76 * k6),
        *args,
    )

    y_new = y + dt * (b1 * k1 + b2 * k2 + b3 * k3 + b4 * k4 + b5 * k5 + b6 * k6)
    ys_new = y + dt * (
        bs1 * k1 + bs2 * k2 + bs3 * k3 + bs4 * k4 + bs5 * k5 + bs6 * k6 + bs7 * k7
    )
...
def solve_ode(f, tspan, y0, method=RungeKutta45, *args, **options):
    ...
    while t[-1] < tf:
        ...
        if adaptive:
            ...
            scaled_err = dt * (np.abs(err) / scale)

            if scaled_err < 1.0:
                t.append(t_c + dt)
                y.append(y_new)
                safety = 0.9
                factor = safety * (1 / scaled_err) ** (1 / 4)
                dt = dt * min(2.0, max(0.5, factor))
            else:
                safety = 0.9
                factor = safety * (1 / scaled_err) ** (1 / 4)
                dt = dt * max(0.5, factor)
        else:
            t.append(t_c + dt)
            y.append(y_new)
...
def n_body(t, y, p):
    ...
    m = p["m"]
    G = p.get("G", 1.0)
    fix = p.get("fix_first", False)
    n = len(m)
    pos = y[: n * dim].reshape(n, dim)

    r = pos[:, None] - pos[None, :]
    r_norm = np.linalg.norm(r, axis=2, keepdims=True)
    r_norm[r_norm == 0] = 1

    acc = -G * (m[None, :, None] * r) / r_norm**3
...
"""
How to find the right atol and rtol
"""
...
def show_anim(t_s, y, dt, y0, trace_length=20, out_time=0.05):
    ...
    for i in range(0, y0.size // d, d):
        ...
        (ph,) = ax.plot(x_t, y_t, "-", color=[0.7, 0.7, 0.7], linewidth=0.7)
...
def plot_trajectories(y, y0, downsample=1, trace_alpha=0.7):
    plt.style.use("dark_background")
    d = 2  # Assuming 2D coordinates
    colors = ["tab:red", "tab:olive", "tab:pink", "tab:cyan", "tab:purple"]

    yd = y[::downsample, :]

    n_bodies = y0.size // (2 * d)

    pos_data = yd[:, : n_bodies * d]

    y0_positions = y0[: n_bodies * d]

    fig, ax = plt.subplots(figsize=(10, 10))

    x_coords = pos_data[:, 0::d]
    y_coords = pos_data[:, 1::d]
    ...
    segments = []
    for i in range(0, n_bodies * d, d):
        seg = np.column_stack((pos_data[:, i], pos_data[:, i + 1]))
        segments.append(seg)

    lc = LineCollection(
        segments, colors=colors[: len(segments)], linewidths=0.7, alpha=trace_alpha
    )
...
euler = np.array([0, 0, 1, 0, -1, 0, 0, 0, 0, 0.8, 0, -0.8])
...
helium_1 = np.array([0, 0, 2, 0, -1, 0, 0, 0, 0, 0.95, 0, -1])
helium_2 = np.array([0, 0, 3, 0, 1, 0, 0, 0, 0, 0.4, 0, -1])
p4 = {
    "m": np.array([1, 1, 1, 1]),
    "G": 1,
    "dimension": 2,
